...
#define AT_PRODUCT_LTEFI_MD1    1
#define AT_PRODUCT_LTEFI_AP1    2
#define AT_PRODUCT_LTEFI_MD2    3
#define AT_PRODUCT_LTEFI_AP2    4
#define AT_PRODUCT_LTEFI_MD3    5
#define AT_PRODUCT_LTEFI_AP3    6
#define AT_PRODUCT_LTEFI_MD4    7
#define AT_PRODUCT_LTEFI_AP4    8

#ifndef AT_PRODUCT
#define AT_PRODUCT              AT_PRODUCT_LTEFI_MD2
#endif

#define __AT_LTEFI_VERSION(_product)    (((_product)+1)/2)
#define __AT_LTEFI_AP(_product)         (0==((_product)%2))
#define __AT_LTEFI_MD(_product)         (1==((_product)%2))

#define AT_LTEFI_VERSION                __AT_LTEFI_VERSION(AT_PRODUCT)
#define AT_LTEFI_AP                     __AT_LTEFI_AP(AT_PRODUCT)
#define AT_LTEFI_MD                     __AT_LTEFI_MD(AT_PRODUCT)
...
#define AT_ROOTFS_MODE_TYPE     AT_ROOTFS_MODE_TYPE_RO
#endif
...
#   define AT_DEV_HD_MASTER     "dev/udisk"
#   define AT_DEV_USB_MASTER    "dev/udisk1110"
#elif AT_PRODUCT==AT_PRODUCT_LTEFI_AP1 || \
      AT_PRODUCT==AT_PRODUCT_LTEFI_AP2
...
#else
#   define AT_DEV_BOOT          AT_DEV_FLASH(1)
#   define AT_DEV_BOOTENV       AT_DEV_FLASH(2)
#endif
#endif
...
#define AT_ENV_BLOCK_COUNT  (AT_ENV_SIZE/AT_ENV_BLOCK_SIZE)
...
#define AT_PCBA_MODEL       "LI31PNM"
#endif

#define AT_MARK_COUNT       (AT_ENV_BLOCK_SIZE/sizeof(unsigned int))

#ifndef AT_INFO_SIZE
#define AT_INFO_SIZE        64
#endif

#define AT_INFO_COUNT_PER_BLOCK     (AT_ENV_BLOCK_SIZE/AT_INFO_SIZE)
#define AT_INFO_COUNT       ((AT_ENV_BLOCK_COUNT-3)*AT_INFO_COUNT_PER_BLOCK)

#ifndef at_kernel_size
#define at_kernel_size      0x8000
#endif

#ifndef at_rootfs_size
#define at_rootfs_size      0x80000
#endif
...
#define AT_VERSION_STRING_SIZE  sizeof(AT_MAX_VERSION_STRING)
...
static inline int
__at_version_atoi(at_version_t * version, char *string)
{
...
at_version_atoi(char *string)
{
    static at_version_t deft = AT_DEFT_VERSION;
    static at_version_t version = AT_DEFT_VERSION;

    if (__at_version_atoi(&version, string)) {
        return &deft;
    } else {
        return &version;
    }
...
#if !defined(__BUSYBOX__) && 2==AT_LTEFI_VERSION
#   define AT_ENV_INITER        \
        static at_env_t __env;  \
        at_control_t at_control = AT_CONTROL_DEFT(&__env, NULL, NULL); \
        os_use_fake_value /* end */
#else
#   define AT_ENV_INITER    os_use_fake_value
#endif
...
enum {
    ...
    __at_mark_private       = 2,
    __at_mark_debug         = __at_mark_private,
    __ar_mark_uptimes       = 3,
    __at_mark_runtime       = 4,
    __at_mark_cid_mid       = 5,
    __at_mark_cid_psn       = 6,
...
static inline void
__at_mark_private_save(unsigned int backup[])
{
    int i;
    
    for (i=__at_mark_private; i<__at_mark_end; i++) {
        backup[i] = at_mark(i);
    }
}

static inline void
__at_mark_private_restore(unsigned int backup[])
{
    int i;
...
{
    unsigned int backup[__at_mark_end] = {0};
    
    __at_mark_private_save(backup);
    os_objzero(__at_mark);
    __at_mark_private_restore(backup);
...
#define at_find_best(_obj, _skips)  ({  \
    int __idx = __at_find_xest(_obj, _skips, __at_downsort, at_obj_is_good); \
    debug_trace("find best:%d", __idx); \
    __idx;                              \
})  /* end */

#define at_find_worst(_obj, _skips) ({  \
    int __idx = __at_find_xest(_obj, _skips, __at_upsort, __at_obj_always_true); \
...
__at_check_version(at_ops_t * ops, char *value)
{
    ...
    if (__at_version_atoi(&version, value)) {
        return -EFORMAT;
    } else {
        return 0;
    }
...
#define AT_MARK_OPS_NAMES \
    __AT_MARK_OPS("ptest/control",  __at_mark_ptest_control,NULL, __at_set_number,  __at_show_number), \
    __AT_MARK_OPS("ptest/result",   __at_mark_ptest_result, NULL, __at_set_number,  __at_show_number), \
    __AT_MARK_OPS("debug",          __at_mark_debug,        NULL, __at_set_number,  __at_show_number), \
    __AT_MARK_OPS("ut",             __ar_mark_uptimes,      NULL, NULL,             __at_show_number), \
    __AT_MARK_OPS("rt",             __at_mark_runtime,      NULL, NULL,             __at_show_number), \
    __AT_MARK_OPS("mid",            __at_mark_cid_mid,      NULL, NULL,             __at_show_number), \
    __AT_MARK_OPS("psn",            __at_mark_cid_psn,      NULL, NULL,             __at_show_number), \
    __AT_MARK_OPS("na",             __at_mark_noauth,       NULL, NULL,             __at_show_number)  \
    /* end */
...
extern int __at_load(int idx);
extern int __at_save(int idx);
#elif defined(__APP__)
#if AT_PRODUCT==AT_PRODUCT_LTEFI_MD1
#define at_open()           0
#define at_close()          0
#define __at_load(_idx)     0
#define __at_save(_idx)     0
#elif AT_PRODUCT==AT_PRODUCT_LTEFI_MD2
static inline int
at_open(void)
{
    __at_fd = __os_file_lock(AT_DEV_BOOTENV, O_RDWR, 0, true);
...
at_close(void)
{
    __os_file_unlock(AT_DEV_BOOTENV, __at_fd);
...
at_save(void)
{
    ...
    for (i = begin; i < AT_ENV_BLOCK_COUNT; i++) {
        if (__at_dirty[i]) {
            err = __at_save(i);
                debug_trace_error(err, "save atenv block %d", i);
            if (err) {
                /* todo: log */
            }
...
__at_init(void)
{
    ...
    err = at_open();
        debug_trace_error(err, "open atenv");
    if (err) {
        return err;
    }
...
__at_fini(void)
{
    ...
    err = at_close();
        debug_trace_error(err, "close atenv");
