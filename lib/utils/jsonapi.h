...
{
    int len, err = 0;
    char *line = (char *)os_zalloc(1+OS_HUGE_LEN);
    if (NULL==line) {
        err = -ENOMEM; goto error;
    }

    len = os_vsnprintf(line, OS_HUGE_LEN, fmt, args);
    if (os_snprintf_is_full(OS_HUGE_LEN, len)) {
        err = -ENOSPACE;
    } else {
...
jobj_file(char *file)
{
    ...
    if (NULL==obj) {
        jsonapi_debug("read json file %s failed", file);
    }
...
jobj_fd(int fd)
{
    ...
    if (NULL==obj) {
        jsonapi_debug("read fd %d failed", fd);
    }
